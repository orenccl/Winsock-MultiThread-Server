...
public :
	...
	inline void  FreeNode( TDNode< DATA_TYPE > *&release_node )
	{
		if( !release_node )
		{
			Logger::Log( "TMPDoublyList< %s >::ReleaseNode   error : release_node = NULL", typeid( DATA_TYPE ).name() );
			return;
		}
		if( NodeUseCount == 0 )
		{
			Logger::Log( "TMPDoublyList< %s >::ReleaseNode   error : NodeUseCount = 0", typeid( DATA_TYPE ).name() );
			return;
		}
