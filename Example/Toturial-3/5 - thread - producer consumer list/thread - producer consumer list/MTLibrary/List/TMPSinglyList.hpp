...
public :
	...
	inline void FreeNode( TSNode< DATA_TYPE > *&release_node )
	{
		if( !release_node )
		{
			Logger::Log( "TMPSinglyList< %s >::FreeNode   error : release_node = NULL", typeid( DATA_TYPE ).name() );
			return;
		}
		if( NodeUseCount == 0 )
		{
			Logger::Log( "TMPSinglyList< %s >::FreeNode   error : NodeUseCount = 0", typeid( DATA_TYPE ).name() );
			return;
		}
